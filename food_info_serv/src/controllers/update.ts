...
import { IFoodParams, IFoodUpdate } from '../interfaces/IFood';
...
export async function updateController(req: Request, res: Response, next: NextFunction) {
  try {    
    ...
    const body = schemaValidator<IFoodUpdate>(req.body as unknown as IFoodUpdate, update_body_schema);

    const update = new UpdateFood();

    await update.update(path.name,body);
